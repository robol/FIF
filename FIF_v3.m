function [IMF,logM] = FIF_v3(f,options,M)


%
%  function IMF = FIF_v2(f,options)
%
...
if nargin < 1,  help FIF_v2; return; end
...
f_pp=f;
f_pp(abs(f)<=10^-18)=[];
maxmins_pp=Maxmins_v3_4(f_pp,options.IF.extensionType);
diffMaxmins_pp=diff(maxmins_pp);
N_pp=length(f_pp);
k_pp = length(maxmins_pp);

if options.plots>=1
    maxmins=Maxmins_v3_4(f,options.IF.extensionType);
    figMask=figure;
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
	...
	% Determine the number of iterations necessary for convergence
	compute_n_its = true;
	
	if compute_n_its
		fh = abs(fftH); fm = abs(ifftA);
		r = @(inStepN) evaluate_residual(fh, fm, inStepN) - options.IF.delta;
		...
		while res > 0
			% More efficient computation, but equivalent to calling r(...)
			r2 = r2 .* omfm;
			r1 = r1 .* omfm;
			omfm = omfm.^2;
			res = sum(r1.^2) ./ sum(r2.^2) - options.IF.delta;
			inStepN = 2 * inStepN;
		end

		% Find the right point by bisection
		jl = floor(inStepN / 2); jr = inStepN;
		rm = res; jm = jr;
		while jr > jl + 1
			jm = floor(.5 * (jl + jr));
			
			% FIXME: The following could probably be made faster
			rm = r(jm);
			if rm < 0
				jr = jm;
			else
...
%         set(figMask,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_FIF_v2.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_FIF_v2.mat'],'IMF','logM','-v7.3');
end
...
function varargout = Maxmins_v3_4(f,extensionType)
% Based on version 3
...
function r = evaluate_residual(fh, fm, j)
	r2 = fh .* (1 - fm).^(j - 1);
	r1 = r2 .* fm;
	r = sum(r1.^2) ./ sum(r2.^2);
end